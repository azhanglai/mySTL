...
// 负责更高级的动态内存管理，包含基本函数、空间配置器、未初始化的存储空间管理
// 一个模板类 auto_ptr
...
namespace laistl {
    ...
    public:
        ...
        ~temporary_buffer() {
            laistl::destroy(buffer, buffer + len);
            free(buffer);
        }
    public:
    ...
    private:
        ...
        void initialize_buffer(const T& value, std::false_type) {
            laistl::uninitialized_fill_n(buffer, len, value);
        }
    private:
        temporary_buffer(const temporary_buffer&);  // 拷贝函数
        void operator=(const temporary_buffer&);    // 赋值拷贝
    };
    ...
    temporary_buffer(ForwardIterator first, ForwardIterator last) {
        try {
            len = laistl::distance(first, last);
            allocate_buffer();
            if (len > 0) {
